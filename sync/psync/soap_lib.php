...
define("PHPR_SYNCWAY_BOTH", "Both");
define("PHPR_SYNCWAY_TOPHPROJEKT", "ToPHProjekt");
define("PHPR_SYNCWAY_FROMPHPROJEKT", "FromPHProjekt");
...
$vcal2acc = array('PUBLIC'=>'group', 'PERSONAL' => 'group', 'private' => 'CONFIDENTIAL');
 ...
 * @return unknown
 */
function syncModule($namespace, &$exchangeset, $loginstring, $user_pw, $sync_date, $delete, $private, $sync_mode) {
    // we have to global all this stuff to figure out what rights the user has
    global $user_ID, $user_acc, $user_group, $sql_user_group;
    global $sync_data, $module, $format, $dbTSnull;
	...
	if (is_null ($sync_mode) 
		|| ($sync_mode != PHPR_SYNCWAY_TOPHPROJEKT 
			&& $sync_mode != PHPR_SYNCWAY_FROMPHPROJEKT)) {
		$sync_mode = PHPR_SYNCWAY_BOTH;
	}
    ...
    if (is_array($exchangeset)) {
        ...
        foreach ($exchangeset as $data) {
            ...
            checkSyncmode($record, $sync_date, $sync_mode);
        }
    ...
    if ($delete) {
	  appDebug(array(__FUNCTION__, __LINE__, "[$module] - delete data from pimp".print_r($pimIDs, true)), 1);
	  deleteData($pimIDs, $sync_date, $sync_mode, $private);
	}

	if ($sync_mode != PHPR_SYNCWAY_TOPHPROJEKT) {
		$ary = preparePimData($namespace, $sync_date, $sync_mode);
	}
 ...
 * @return boolean
 */
function SyncCalendar(&$exchangeset, $username, $password, $syncdate, $delete, $private, $syncMode) {
    global $module;
    appDebug(__FUNCTION__.":".__LINE__.": SyncCalendar(&$exchangeset, $username, $password, $syncdate, $delete)", 4);
    $module = 'termine';
    return syncModule('SyncCalendar', $exchangeset, $username, $password, $syncdate, $delete, $private, $syncMode);
}
 ...
 * @return boolean
 */
function SyncContacts(&$exchangeset, $username, $password, $syncdate, $delete, $private, $syncMode) {
    global $module;
    $module = 'contacts';
    return syncModule('SyncContacts', $exchangeset, $username, $password, $syncdate, $delete, $private, $syncMode);
}
 ...
 * @return boolean
 */
function SyncNotes(&$exchangeset, $username, $password, $syncdate, $delete, $private, $syncMode) {
    global $module;
    $module = 'notes';
    return syncModule('SyncNotes', $exchangeset, $username, $password, $syncdate, $delete, $private, $syncMode);
}
 ...
 * @return boolean
 */
function SyncTodos(&$exchangeset, $username, $password, $syncdate, $delete, $private, $syncMode) {
    global $module;
    $module = 'todo';
    return syncModule('SyncTodos', $exchangeset, $username, $password, $syncdate, $delete, $private, $syncMode);
}
...
function ListProjects( $loginstring, $user_pw) {
    ...
    $where = "(acc like 'system' or ((von = ".(int)$user_ID." or acc like 'group' or acc like '%\"$user_kurz\"%') and $sql_user_group)) $where";

    $query="select ID, name from ".DB_PREFIX.$tablename['projects']." where $where";
...
function SetTimeCard($exchangeset, $loginstring, $user_pw, $syncdate, $delete) {
    ...
    global $user_ID, $user_kurz, $sql_user_group, $module;
    global $user_group, $user_acc, $dbIDnull, $dbTSnull, $version;
    ...
    @include_once(PATH_PRE.'lib/lib.inc.php');
    //@include_once(PATH_PRE."timecard/timecard_date.inc.php");
    check_role("timecard");
    ...
    foreach ($exchangeset as $entry) {
        ...
        $sqlstring = "INSERT INTO ".DB_PREFIX."timecard
                                 (users, datum, anfang)
                          VALUES (".(int)$user_ID.",'".strip_tags($datum)."', ".(int)$time.")";
...
function SyncResync(&$exchangeset, $loginstring, $user_pw) {
    global $user_ID, $user_group, $user_acc, $dbIDnull, $dbTSnull;
 ...
 * @param string $sync_date
 */
function checkSyncmode($pim_record, $sync_date, $syncMode) {
    global $module, $format, $user_ID, $user_group, $sync_data, $user_acc;

    $syncID = $pim_record[0];
    // fetch sync record

	$modified = ($syncMode == PHPR_SYNCWAY_BOTH) ? " AND modified >= $sync_date" : " ";
...
$modified
	...
	if($syncMode == PHPR_SYNCWAY_TOPHPROJEKT) {
		appDebug(array(__FUNCTION__, __LINE__, "[$module] Syncway client -> phprojekt"), 1);
	}
	if($syncMode == PHPR_SYNCWAY_FROMPHPROJEKT) {
		appDebug(array(__FUNCTION__, __LINE__, "[$module] Syncway phprojekt -> client"), 1);
	}
	if($syncMode == PHPR_SYNCWAY_BOTH) {
		appDebug(array(__FUNCTION__, __LINE__, "[$module] Syncway client <-> phprojekt"), 1);
	}
    ...
    if ($row[0] > 0) {
		...
		if ((($last_sync >= $phprojekt_mod && $pim_checksum != $sync_checksum)
			 || $syncMode == PHPR_SYNCWAY_TOPHPROJEKT)
			&& $syncMode != PHPR_SYNCWAY_FROMPHPROJEKT
    ...
    } else if ($syncMode != PHPR_SYNCWAY_FROMPHPROJEKT) {
        //
...
function insertSyncrecord($phprojekt_ID, $sync_ID, $checksum, $sync_type='outlook', $sync_version='0.1') {
    ...
    $query = "INSERT INTO ".DB_PREFIX."sync_rel
        (user_ID, sync_type, sync_ID, sync_module, phprojekt_ID, phprojekt_module, created, modified, sync_checksum, sync_version)
            VALUES
        (".(int)$user_ID.", '".strip_tags($sync_type)."', '".strip_tags($sync_ID)."', '".qss($module)."', ".(int)$phprojekt_ID.", '".qss($module)."', '$dbTSnull', '$dbTSnull', '".strip_tags($checksum)."', '".strip_tags($sync_version)."')";
    appDebug(array(__FUNCTION__, __LINE__, "SQL query:\n$query\n"), 8);
    $result = db_query($query);
}
...
function updatePhprojektData($ID, $pim) {
    ...
    switch ($module) {
        ...
        case 'notes':
            ...
            break;
        case 'todo': // array( '', 'remark', 'note', 'anfang', 'deadline', 'progress', 'acc', 'datum')
            $sql_string[] = $format[$module][1]."='".addslashes(utf8_decode($pim[1]))."'";
            $sql_string[] = $format[$module][2]."='".addslashes(utf8_decode($pim[2]))."'";
...
function convertPhprojektData(&$row) {
    ...
    switch ($module) {
        ...
        case 'notes':
            ...
            break;
        case 'todo':
            $values[] = escapeHTML(utf8_encode($row[1]));
            $values[] = utf8_encode($row[2]);
            ...
            $values[] = $row[7] != '' ? substr($row[7],6,2).'.'.substr($row[7],4,2).'.'.substr($row[7],0,4) : '';
...
function insertPimRecord($pim) {
    ...
    // run db query
    $query = "INSERT INTO ".DB_PREFIX.qss($module)."
                          (".implode(',',$sql_fields).", sync1,       sync2)
                   VALUES (".implode(',',$sql_values).", '$dbTSnull', '$dbTSnull')";
    appDebug(array(__FUNCTION__, __LINE__, "SQL query:\n$query\n"), 8);
    $result = db_query($query);
...
function getPhprojektWriteAccess($withprivate = true) {
    ...
    appDebug(array(__FUNCTION__, __LINE__, "Write Access:\n".var_export($ret, true)."\n"), 8);
 ...
 * @return array
 */
function preparePimData($namespace, $sync_date, $syncMode) {
    global $module, $format, $user_ID, $sync_data, $user_acc, $dbTSnull;
    ...
    // fetch values
	if ($syncMode != PHPR_SYNCWAY_FROMPHPROJEKT) {
		$syncDateCheck = "((sync2 > $sync_date) or (sync2 = '$dbTSnull')) AND ";
    } else { 
		$syncDateCheck = ""; 
	}
    ...
    $query = "SELECT ".implode(', ', $phprojekt_fields)."
               ...
               WHERE 
                    $syncDateCheck
                    ID IN ('".implode("','", $user_acc[checkPhprojekt2Pim()])."')";
    appDebug(array(__FUNCTION__, __LINE__, "SQL query:\n$query\n"), 8);
 ...
 * @param string $sync_date
 */
function deleteData(&$pimIDs, $sync_date, $syncMode, $private) {
    ...
    if (count($pimIDs)==0) {
        appDebug(array(__FUNCTION__, __LINE__, "[$module] - delete entry from pimIDs array"), 1);
        $pimIDs[0] = '-1';
    }
    ...
    $res[0] = array();
    while ($row = db_fetch_row($result)) {
		if (in_array($row[1], $user_acc['write'])) {
			$res[0][] = $row[0];
			$res[1][] = $row[1];
		}
    }
    if (count($res[0])>0 && $syncMode != PHPR_SYNCWAY_FROMPHPROJEKT) {
