 ...
 * @version    $Id: notes_selector_data.php
 */
...
$formdata['_ID']       = $ID;
$formdata['_mode']     = $mode;
$formdata['_selector_name'] = 'note_selector_';

// Selector for contacts (List view - contact)
if (isset($_REQUEST['action_form_to_list_notes_contact_selector']) ||
    ...
    $_SESSION['notedata']['formdata'] = $formdata;
    ...
    if (isset($_REQUEST['action_list_notes_contact_selector_to_form_ok'])) {
        // pressed okay
        $selector = xss_array($_POST[$_SESSION['notedata']['formdata']['_selector_name']."srcs"]);
        put_filter_value('contact','exact',$selector[0]); 
    }
    // okay & cancel
    unset($_SESSION['notedata']['formdata']);
...
// Selector for contacts (Form)
    ...
    // pressed okay
        $selector = xss_array($_POST[$_SESSION['notedata']['formdata']['_selector_name']."srcs"]);
        $_SESSION['notedata']['formdata']['contact'] = $selector[0];
    }
    // okay & cancel
    $formdata = $_SESSION['notedata']['formdata'];
    unset($_SESSION['notedata']['formdata']);
    unset($_REQUEST['filterform']);
...
// Selector for project (Form)
    ...
    if (isset($_REQUEST['action_project_selector_to_form_ok'])) {
        // pressed okay
        $selector = xss_array($_POST[$_SESSION['notedata']['formdata']['_selector_name']."srcs"]);
        $_SESSION['notedata']['formdata']['project'] = $selector[0];
    }
    // okay & cancel
    $formdata = $_SESSION['notedata']['formdata'];
    unset($_SESSION['notedata']['formdata']);
...
// selector for access (Form)
    ...
    if (isset($_REQUEST['action_access_selector_to_form_ok'])) {
        // pressed okay
        $selector = xss_array($_POST[$_SESSION['notedata']['formdata']['_selector_name']."dsts"]);
        if (is_array($selector)) {
            foreach($selector as $k => $u_id) {
                $_SESSION['notedata']['formdata']['persons'][] = slookup('users','kurz','ID',$u_id,'1');
            }
        } else $_SESSION['notedata']['formdata']['persons'] = array();
    }
    // okay & cancel
    $formdata = $_SESSION['notedata']['formdata'];
    unset($_SESSION['notedata']['formdata']);
