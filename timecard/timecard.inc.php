 ...
 * @version    $Id: timecard.inc.php,v 1.23 2007-12-12 09:48:48 nina Exp $
 */
...
function timecard_parse_template($template, $project_id, $values,$indent=0) {
    ...
    switch ($template) {
        case "project_time_form":
            ...
            $html = "<tr class='book3'>
	                  <td class='book3' style='padding-left:10px;' $col1_span>";
        ...
        case 'project_time_modulehead':
            ...
            $html = "<tr class='book1'>
	                  <td class='book1' $col1_span>";
            for ($i=0; $i < $indent; $i++) { $html.= "&nbsp;&nbsp;&nbsp;&nbsp;"; }
            $html.="<img src='".IMG_PATH."/t.gif' width='9' height='9' />";
            $mod=$values["M_TYPE"];
            if (!$arrproj1[$mod][$project_id]) {
                $no = "open".$mod.$project_id;
                $html.= "<a href='timecard.php?submode=$submode&date=$date&element2_mode=modopen&element2_ID=$project_id&mod=$mod'>
         		  ".tree_icon("parent","name='$no' style='border-style:none;'")."</a>&nbsp; ";
            }
            else {
                $nc= "close".$mod.$project_id;
                $html.= "<a href='timecard.php?submode=$submode&date=$date&element2_mode=modclose&element2_ID=$project_id&mod=$mod'>
