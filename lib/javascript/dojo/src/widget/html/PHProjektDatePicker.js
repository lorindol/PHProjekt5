...
dojo.provide("dojo.widget.html.PHProjektDatePicker");
...
dojo.widget.defineWidget(
	"dojo.widget.html.PHProjektDatePicker",
	dojo.widget.HtmlWidget,
	{
		classConstructor: function() {
			// mixin dojo.widget.DatePicker non-demoninational code
			dojo.widget.PHProjektDatePicker.call(this);
		...
		templatePath:  dojo.uri.dojoUri("src/widget/templates/PHProjektHtmlDatePicker.html"),
		templateCssPath:  dojo.uri.dojoUri("src/widget/templates/PHProjektHtmlDatePicker.css"),
		...
		initData: function() {
			...
			if(this.storedDate && (this.storedDate.split("-").length > 2)) {
				this.date = dojo.widget.PHProjektDatePicker.util.fromRfcDate(this.storedDate);
			} else {
			...
			var tempSaturday = dojo.widget.PHProjektDatePicker.util.initFirstSaturday(this.date.getMonth().toString(), this.date.getFullYear());
            ...
            var count = 0;
            for (var i = dojo.date.firstSaturday - 7; count < 7; i++) {
                var pos = i - 1;
                if( pos > 6) { pos = i-8;}
                this.setDatesLabel(count,pos);
                count++;
            }
		},
		
		setDatesLabel: function(indexTd, indexTxt) {
	        var datesNodes = this.datesContainerNode.getElementsByTagName("td");
			var currentDateNode = datesNodes.item(indexTd);
			currentDateNode.innerHTML = dojo.date.days[indexTxt];
		},
		...
		incrementMonth: function(evt) {
			...
			var tempSaturday = dojo.widget.PHProjektDatePicker.util.initFirstSaturday(month.toString(), year);
			this.firstSaturday.year = tempSaturday.year;
			this.firstSaturday.month = tempSaturday.month;
			this.firstSaturday.date = tempSaturday.date;
			this.initUI();
		...
		incrementYear: function(evt) {
			...
			var tempSaturday = dojo.widget.PHProjektDatePicker.util.initFirstSaturday(this.firstSaturday.month.toString(), year);
			this.firstSaturday.year = tempSaturday.year;
			this.firstSaturday.month = tempSaturday.month;
			this.firstSaturday.date = tempSaturday.date;
			this.initUI();
		...
		onIncrementDate: function(evt) {
			dojo.unimplemented('dojo.widget.html.PHProjektDatePicker.onIncrementDate');
		},
		...
		onSetDate: function(evt) {
			...
			this.setDate(dojo.widget.PHProjektDatePicker.util.toRfcDate(this.date));
			this.initUI();
		}
	}
);
