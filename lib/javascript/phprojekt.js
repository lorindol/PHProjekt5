...
/*
 * Check if a the current selected option of a select item exists
 * and is not 'NULL'
 *
 * @param string frm  - name of the form
 * @param string fld  - the name of the <select> element
 * @param string alertText  - msg to display if check fails
 * @return boolean
 */
function checkSelectedOption(frm, fld, alertText) {
	var selected = getSelectedOption(frm, fld);
...
function checkSubprojectAndCostunit(frm, ename, pname, msg) {
	var el = document.forms[frm].elements[ename];
	var parent = document.forms[frm].elements[pname][[document.forms[frm].elements[pname].selectedIndex]].value;

	if (el) {
		var value = (el.length) ? el[document.forms[frm].elements[ename].selectedIndex].value : el.value;
		if (value == 'NULL' && (parent == 0 || isNaN(parent))) {
			alert(msg);
			return false;
		}
	}
 ...
 /*
 * Checks if multiple form elements (e.g arrays)
 * with same name have the same entries.
 *
 * @param string frm - name of the form
 * @param string ename - name of the form element
 * @return boolean - true if duplcates exists
 */
function checkDuplicate (frm, ename, msg){
	var el = document.forms[frm].elements[ename];
	for (i = 0; i < el.length ; i++) {
		for (j = 0; j < el.length; j++) {
			if (el[j].value == el[i].value && j != i) {
				alert(msg);
				return true;
			}
		}
 ...
 *
 */
function getSum (frm, ename){
	var el = document.forms[frm].elements[ename];
	var sum = 0.0;
	
	if (el.length) {
		for (i = 0; i < el.length ; i++) {
			sum += parseFloat(el[i].value);
		}
	} else if(el) {
		return el.value;
    }
...
function checkProjectFormSum (frm, ename, msg){
	if (Math.abs(getSum(frm, ename) - 100.0) > 0.01
	 && Math.abs(getSum(frm, ename)) > 0.01)
	 {
		alert(msg);
		return false;
	}
...
/*
 * Returns the value of select option or false if not available
 *
 * @param string frm  - name of the form
 * @param string fld  - the name of the <select> element
 * @return mixed
 */
function getSelectedOption(frm, fld) {
	var sel = document.forms[frm].elements[fld];
	if (!isNaN(sel.selectedIndex)) {
...
/*
 * Clones an element of a DOM tree
 * Thats used by projects_forms.php
 *
 */
function cloneItem(frm, fld, createbreak, sibling) {
	var parent   = document.getElementById(fld);
	var newNode  = document.getElementById(fld).cloneNode(true);
    newNode.id   = '';
    if (newNode.hasChildNodes) {
    	for(i = 0; i < newNode.childNodes.length; i++) {
            if (undefined != newNode.childNodes[i].disabled) {
        		newNode.childNodes[i].disabled = '';
            }
    	}
    }
    if(createbreak) {
    	parent.parentNode.insertBefore(document.createElement("br"), parent.nextSibling);
     }
...
/*
 * return a digit with 2 numbers
 * add 0 if is < 10
 *
 * @param string x - digit
 */
 ...
 *
 * @param string m - Menu name
 * @param string i - Field name
 * @param obj c    - TH tag
 * @return void
 */
...
/**
 * Make a js link for open a popup
 * 
...
function doLink(url, target, msg) {
	...
	switch (target) {
		case "_blank":
		window.open(url,'new','left=5px,top=5px,width=980px,height=540px,scrollbars=1,resizable');
		break;
		case "_top":
		window.open(url,'new','left=5px,top=5px,width=980px,height=540px,scrollbars=1,resizable');
		break;
...
/**
 * Make a js link for open a popup
 * Add all the lines selected to the link
 * 
 * @param string url    - Url for the popup
 * @param string target - _blank/_top 
 * @param string msg    - Confirm message
...
var showbox=false;
/**
 * Display the alt data
 * 
 * @param string text     - HTML content
 * @param string width    - Width of the alt table
 * @param string heigth   - Height of the alt table
 * @param obj id          - alt table object
 * @return void
 */
...
/**
 * Get the position of the alt table
 * 
 * @param event e - Mouse event
 * @return void
 */
...
function chkFloat(frm, fld, txt) {
	var string = document.forms[frm].elements[fld].value;
	if (string != "") {
		var searchfor = /^((\d+(\,\d{1,2})?)|((\d*\,)?\d{1,2}))$/;
		var result = searchfor.test(string);
		if(result==false) {
			alert(txt);
			document.forms[frm].elements[fld].focus();
...
/*
 * Change the target/action of the form
 *
 * @param object theForm - form object
 * @return void
 */
 ...
 * @return void
 */
function manage_filters(path_pre,filtermodule,module,mode,ID,expert) {
    var url = path_pre+"lib/dbman_filter_pop.php?module="+filtermodule+"&opener="+module+"&mode="+mode+"&ID="+ID;
    if (expert == 1) {
        url = url + "&expert=1";
    }
    var title = "Filter";
    var parameters = "left=100,top=50,width=600,height=570,toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=1";
...
/*
 * Open a new window for edit saved settings
 *
 * @param string data - All data to make the link
 * @return void
 */

function manage_saved_settings(path_pre,module,back,mode,mode2,opener) {
    var url = path_pre+"lib/save_selections_pop.php?module="+module+"&back="+back+"&mode="+mode+"&mode2="+mode2+"&opener="+opener;
    var title = "_blank";
    var parameters = "left=100,top=100,width=700,height=500,toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=0,resizable=1";
    make_new_win(url,title,parameters);
}
/*
 ...
 * @return void
 */
function manage_related_object(path_pre,module_alias,projekt_ID,contact_ID,sid) {
    if (module_alias != 'mail') {
        var url = path_pre+module_alias+"/"+module_alias+".php?mode=forms&projekt_ID="+projekt_ID+"&contact_ID="+contact_ID+"&justform=1"+sid;
    } else {
        var url = path_pre+module_alias+"/"+module_alias+".php?mode=send_form&form=email&projekt_ID="+projekt_ID+"&contact_ID="+contact_ID+"&justform=1"+sid;
    }
...
/*
 * Open a new window using the url input
 *
 * @return void
 */
function go_web() {
 ...
 * Make a mail link
 *
 * @param object field  - Object field input
 * @param string adress - Mail address
 * @param int quickmail - Use quickmail?
 * @return void
 */
...
/*
 * Open a new window with a phone link
 *
 * @param string phonetype   - Phone type
