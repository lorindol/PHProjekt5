 ...
 * @access public
 */
function organisationsfetch_fields($options,$preselect) {
    ...
    if (isset($filter_answer) && !empty($filter_answer)) {
        ...
        foreach($filter_answer as $k => $v) {
            ...
            $query = "SELECT ".$options['ID'].", o.name
                        FROM ".DB_PREFIX."organisations AS o
                       WHERE ".$options['ID']." = ".xss($k)."
                         AND o.is_deleted is NULL";
 ...
 * @access public
 */
function organisationsdisplay_filters1($options, $object, $name, $getprm=array()) {
    global $filters, $organisationsextras, $sid, $getstring;
    ...
    if (!empty($organisationsextras)) {
        ...
        foreach ($organisationsextras as $key=>$val) {
            $val = xss_array($val);
 ...
 * @access
 */
function organisationsparse_filters(&$object) {
