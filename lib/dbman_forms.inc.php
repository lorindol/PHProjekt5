 ...
 * @version    	$Id: dbman_forms.inc.php,v 1.136 2008-02-28 08:04:17 albrecht Exp $
 */
...
/**
 * Output of all elements of a form
 * uses the function form_element to display
 * the different types of form elements like text, textarea  etc.
 *
...
function build_form($fields) {
    ...
    foreach ($new_fields as $tabs => $tab_fields) {
        ...
        $tabs_name_str= slookup('modules','index_name,name,form_cols','ID',$tabs);
        $tabs_name=explode(',',$tabs_name_str);
        $tabs_name_out = $tabs_name[1] <> '' ? enable_vars($tabs_name[1]) :__('Basis data');
        if ($tabs_name[2] > 0) {
            $nbr_cols = $tabs_name[2];
        }
        else $nbr_cols = 2;
 ...
 *
 * @param array  	$field         		- All the field data
 * @param string 	$field_name    	- Name of the field
 * @param string 	$force_newline 	- Without use
 * @return string               				HTML output
 */
function form_element($field, $field_name, $force_newline) {
    ...
    if (!is_array($field['value'])) {
    	if (PHPR_SUPPORT_HTML and (isset($module) && isset($_SESSION['show_html_editor']["$module"]) && $_SESSION['show_html_editor']["$module"] == 1) and $field['form_type'] == 'textarea') {
    		$field['value'] = stripslashes(xss_purifier($field['value']));
    	}
    	else $field['value'] = stripslashes(xss($field['value']));
    } else {
    	if (PHPR_SUPPORT_HTML and (isset($module) && isset($_SESSION['show_html_editor']["$module"]) && $_SESSION['show_html_editor']["$module"] == 1) and $field['form_type'] == 'textarea') {
    		$field['value'] = xss_purifier($field['value']);
    	}
    	$field['value'] = xss_array($field['value']);
    }
...
if(defined('IN_DEV_MODE') && !in_array($field['form_type'], array('textarea','text','email','phone','url','select_category','select_values','date','contact_create','contact','project','user_show','timestamp_modify','userID','timestamp_create','display', 'authorID', 'select_sql', 'upload', 'timestamp_show'))){
    ...
    // Return from the db to the originial value
    if (!is_array($field['value'])) {
    ...
    switch ($field['form_type']) {
		...
		case 'display_geshi': 
			if ($GLOBALS['fields']['geshi']['value']) {
                include_once(PATH_PRE.'lib/geshi.php');
                $geshi =& new GeSHi($GLOBALS['fields']['remark']['value'], $GLOBALS['fields']['geshi']['value']); 
				$geshi->enable_line_numbers(GESHI_NORMAL_LINE_NUMBERS);
				$output1 .= '<fieldset>'.sprintf('<label class="%s" for="%s">%s</label>%s',
                                $label_style,
                                $field_name,
                                enable_vars($field[form_name]),
                                $geshi->parse_code()
        ...
        case 'checkbox':
            ...
            $output1.= enable_vars($field[form_name])."</label>";
            $output1.= "<input class='".$field_size."' style='width:15px;' type='checkbox' name='".$field_name."' value='yes'";
            if ($field['form_tooltip'] <> '') { $output1.= " title='".$field['form_tooltip']."'"; }
        ...
        case 'select_values_text':
            ...
            $output1.= "</select>\n";
            $output1.= "<label class='form$ff; ' for='{$field_name}_other'>Other</label>";
            if (!$found) {
                $value = $field['value'];
            }
            else {
        ...
        case 'upload':
            ...
            foreach ($files as $value){
                ...
                if ($filename <> '') {
                    ...
                    if (!$read_o) {
                        $output1.= "<a href='$module.php?mode=data&amp;delete_file=1&amp;ID=$ID&amp;file_field_name=".$field_name."&amp;".$getstring."&amp;csrftoken=".make_csrftoken()."'><img src='".IMG_PATH."/r.gif' height=7 alt='".__('Delete')."' border=0></a>";
                    }
                    $output1 .= ")\n";
                }
            }
        ...
        case 'time':
            ...
            $output1 .= sprintf('<label class="%s" for="%s">%s</label>
                                <select class="" id="%s_hour" name="%s_hour" %s %s>
                                %s
                                </select>
             					<select class="" id="%s_minute" name="%s_minute" %s %s>
                                %s
                                </select>',
                            $label_style,
                            $field_name,
...
/**
 * Provide values from other fields and formulas
 *
 * @param string 	$content 	- Value of the field
 ...
 *
 * @param array		$f 		- Value of the field
 * @return string  			Converted value
 */
function f2($f) {
    $y = $f[1];
...
/**
 * Return size of input element
 * especially if we have a rowspan/colspan > 1
 *
 * @param string 	$field 	- Array with all the field data
 * @return string       			HTML output for size
 */
 ...
 *
 * @param string 	$regexp     	- Regular expresion
 * @param string 	$field_name 	- Name of the field
 * @return string            			HTML output for onblur
 */
 ...
 *
 * @param string 	$mode     	- Type of the button: create/modify/copy
 * @return string          			HTML output
 */
...
function get_action_buttons($read_o, $ID, $justform, $module, $deleteable, $sid, $copy=true) {
    ...
    if (!$read_o && $user_role > 1 && $copy == true && $ID > 0) {
        $buttons[] = array('type' => 'link', 'href' => $module.".php?mode=forms&ID=".$ID."&copyform=1&justform=".$justform, 'text' => __('copy'), 'active' => false);
    }
...
function generate_output($out,$subtabs=false, $selected='') {
    ...
    switch($_SESSION['settings']['output_type']) {
        ...
        $outstring.='<div id="dojoParentTab" dojoType="AccordionContainer" labelNodeClass="label" containerNodeClass="accBody" class="dojoAccordion">';
        $i =0;
        foreach ($out as $value) {
            list($name, $content) = $value;
            switch($name) {
