...
*
* @package    admin
* @subpackage module designer
* @author     Albrecht Guenther, $Author: gustavo $
* @licence    GPL, see www.gnu.org/copyleft/gpl.html
* @copyright  2000-2006 Mayflower GmbH www.mayflower.de
* @version    $Id: module_designer.php,v 1.57 2007-05-31 08:10:09 gustavo Exp $
*/
...
$modules_available['organisations'] = __('Organisations');
...
// 'related_table' => __('Related Table'),
'time' => __('Time'));
...
switch ($action) {
    ...
    case 'submit':
        ...
        // crate new field
            ...
            if ($form_type == 'related_table'){
                $query = 'CREATE TABLE `'.DB_PREFIX.$form_select.' (
                         `ID` '.$db_int8_auto[PHPR_DB_TYPE].',
                         `'.$form_regexp.'` '.$db_int8[PHPR_DB_TYPE].',
                         `'.$db_name1.'_ID` '.$db_int8[PHPR_DB_TYPE].',
                          PRIMARY KEY  (`ID`));';
                $result = db_query($query) or db_die();
            }
            else if ($form_type == 'textarea') $fieldtype = $db_text[PHPR_DB_TYPE];
           ...
           (  db_table,   db_name,   form_name, form_tab,   form_type,   form_tooltip,          form_colspan  ,        form_rowspan  ,  form_regexp,   form_default,   form_select,         form_length,       list_alt,   filter_show, db_inactive)
    values ('".qss($db_table)."','".qss($db_name1)."','".xss($form_name)."','$form_tab','".qss($form_type)."','".qss($form_tooltip)."',".(int)$form_colspan.",".(int)$form_rowspan.",'".qss($form_regexp)."','".qss($form_default)."','".qss($form_select)."',".(int)$list_length.",'".qss($list_alt)."','".qss($filter_show)."',0)";
            $result = db_query($query) or db_die();
        }
        list_values();
        break;
...
function edit($ID) {
    global $modules_available, $form_types, $sid, $db_table, $colsByTab;

    // start form
    echo "<form action='module_designer.php' name='formmd' method='post'>
    <fieldset>
  ...
  ";
    ...
    if ($ID > 0) {
        $result = db_query("select ID, db_table, db_name, form_name, form_type, form_tooltip,
                               ...
                               form_colspan, form_rowspan, form_length, form_tab
                          from ".DB_PREFIX."db_manager
                         where ID = ".(int)$ID) or db_die();
    ...
    // tab of field
    echo "<tr><td>".__('Field Tab')."</td><td>\n";
    echo "<select name='form_tab' onChange='updateNumberOfCols()'>";
    echo "<option value=''";
    if(!$row[14])  echo " selected='selected'";
    echo ' >'.__('Basic Data').'</option>';
    
    $module_ID= get_application_module_ID($db_table);
    
    $query = "SELECT ID, name, form_cols FROM ".DB_PREFIX."modules
                            WHERE module_type='forms' 	 
                            AND parent = ".(int)$module_ID." 	 
                            ORDER BY position"; 	 
    
    $result_tabs = db_query($query) or db_die();
    
    while ($row_tabs= db_fetch_row($result_tabs)) {
        
        echo "<option value='$row_tabs[0]'";
        if ($row_tabs[0] == $row[14]) {
            echo ' selected="selected"';
        }
        echo' >'.enable_vars($row_tabs[1]).'</option>';
        
        // we will save the cols
        $tempID = $row_tabs[0];
        $colsByTab[$tempID] = $row_tabs[2];
    }
    echo "</td><td>".__('Select the tab the field should be displayed in.')."</td></tr>\n";
...
updateNumberOfCols();
function updateNumberOfCols() {
    var selectedOption = 0;
    var valuesByTab = Array();
    var tempValue = 0;
    <?
    foreach ($colsByTab as $key => $value) {
        echo "valuesByTab[".(int)$key."] = ".(int)$value.";\n";
    }
    ?>
    selectedOption = document.formmd.form_tab.value;
    
    tempValue = document.formmd.form_colspan.value;
    
    if (selectedOption == '') {
        selectedOption = 0;
    }
    
    document.formmd.form_colspan.options.length = valuesByTab[selectedOption];
    for (i=0;i < valuesByTab[selectedOption]; i++) {
        document.formmd.form_colspan.options[i] = new Option(i + 1);
    }
    
    if (tempValue <= valuesByTab[selectedOption]) {
        document.formmd.form_colspan.value = tempValue;
    }
