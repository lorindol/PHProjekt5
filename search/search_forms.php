 ...
 * @version    $Id: search_forms.php,v 1.39 2008-01-02 22:14:17 gustavo Exp $
 */
    ...
    if ($row[0] > 0) {
        ...
        while ($row = db_fetch_row($result)) {
            if ($hits <= $maxhits) {
                ...
                tr_tag($ref,"parent.");
                $tbody .= "<tr><td><a href=\"$ref\" target=\"_top\">".html_out($row[1])."</a></td>\n";
                $tbody .= "<td scope=\"row\">$row[2]</td></tr>\n";
            }
            $hits++;
        }
        $tmp .= "
        ...
        $tmp .= "<tbody>".$tbody."</tbody></table><br />\n";
...
if (($gebiet == 'mails' or $gebiet == 'all') and PHPR_QUICKMAIL == 2 and check_role('mail') > 0) {
    ...
    if (isset($_POST['use_filters']) && ($_POST['use_filters'] == "on") ) {
        $where.= main_filter('', '', '', '', 'mail','','');
    }

    $query= "SELECT ID, subject, body, sender
               FROM ".DB_PREFIX."mail_client
                    ".special_sql_filter_flags('mail', xss_array($_POST))."
                    $where
                    AND is_deleted is NULL
                    ".special_sql_filter_flags('mail', xss_array($_POST), false)."
                AND von = ".(int)$user_ID;
    ...
    if ($row[0] > 0) {
        ...
        while ($row = db_fetch_row($result)) {
            if ($hits < $maxhits) {
                ...
                $remark = nl2br(html_out(substr($row[2],0,300)));
                if(strlen($remark) > 300) { $remark = $remark."..."; }
                $tbody .= "<td>".htmlentities($row[3])."&nbsp;</td><td>$remark&nbsp;</td></tr>\n";
            }
            $hits++;
        }
        $tmp .= "
...
if (($gebiet == 'bookmarks' or $gebiet == 'all') and PHPR_BOOKMARKS and check_role('bookmarks') > 0) {
    $hits = 0;
    $fields = array('url'         => array('filter_show' => 1),
                    'bezeichnung' => array('filter_show' => 1),
                    'bemerkung'   => array('filter_show' => 1));

    $where = build_where($fields);

    if (isset($_POST['use_filters']) && ($_POST['use_filters'] == "on") ) {
        $where.= main_filter('', '', '', '', 'bookmarks','','');
    }
...
// results for cost
if (($gebiet == 'costs' or $gebiet == 'all') and PHPR_COSTS and check_role('costs') > 0) {
    $hits = 0;
    $fields = array(    'name' => array('filter_show' => 1),
                        'remark' => array('filter_show' => 1),
                        'amount' => array('filter_show' => 1));

    $where = build_where($fields);

    if (isset($_POST['use_filters']) && ($_POST['use_filters'] == "on") ) {
        $where.= main_filter('', '', '', '', 'costs','','');
    }

    $query = "SELECT ID, name, remark, amount
                FROM ".DB_PREFIX."costs
                     ".special_sql_filter_flags('costs', xss_array($_POST))."
                     $where
                     ".special_sql_filter_flags('costs', xss_array($_POST), false)."
                 AND (von = ".(int)$user_ID." OR gruppe = ".(int)$user_group.")
                 AND is_deleted is NULL";
    $result = db_query($query) or db_die();
    $row = db_fetch_row($result);
    if ($row[0] > 0) {
        ...
        $tmp = "
        <table class='relations'>
            <caption>".__('Costs')."</caption>
            <thead>
                <tr>
                    <td title='".__('Name')."'>".__('Name')."</td>
                    <td title='".__('Remark')."'>".__('Remark')."</td>
                    <td title='".__('Amount')."'>".__('Amount')."</td>
                </tr>
            </thead>";
        $tbody = '';
        while ($row = db_fetch_row($result)) {
            if ($hits < $maxhits) {
                $ref = "../index.php?module=costs&amp;mode=forms&amp;ID=$row[0]$sid";
                tr_tag($ref,"parent.");
                // subject with link
                $tbody .= "<tr><td scope=\"row\" class=\"column-1\"><a href=\"$ref\" target=\"_top\">$row[1]&nbsp;</a></td>\n";
                $remark = nl2br(html_out(substr($row[2],0,300)));
                if(strlen($remark) > 300) { $remark = $remark."..."; }
                $tbody .= "<td>$remark </td><td>$row[3]</td></tr>\n";
            }
            $hits++;
        }
        $tmp .="
        ...
        $ou .= get_no_hits_found_message('Costs');
    }
}
// *********
